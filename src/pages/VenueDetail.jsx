...
export function VenueDetail({
  ...
  const errors = {
    ...
    strasse:
      form.strasse.trim().length > 0 &&
      form.strasse.trim().length < 3,

    plzort:
      form.plzort.trim().length > 0 &&
      form.plzort.trim().length < 3,
    ...
    try {
      ...
      try {
        if (!guestMail) {
          ...
          if (!g?.success) {
            console.error("Guest email send failed:", g?.error || "EmailJS not configured");
          } else {
      ...
      try {
        if (!venueMail) {
          ...
          if (!vn?.success) {
            console.error("Host email send failed:", vn?.error || "EmailJS not configured");
          } else {
          ...
          console.warn("Host email skipped — no valid venue email address:", venueMail.an);
        }
      } catch (err) {
          ...
          >
            ...
            >
              {t("venue.confirmation.title", {
                defaultValue: "Du sitzt mit am Tisch",
              })}
            </div>
            ...
            >
              {r.persons === 1
                ? t("venue.confirmation.oneSeat", {
                    defaultValue: "1 Platz",
                  })
                : t("venue.confirmation.multipleSeats", {
                    count: r.persons,
                    defaultValue: `${r.persons} Plätze`,
                  })}{" "}
              {t("venue.confirmation.atTable", {
                defaultValue: "am Mischtisch im",
              })}{" "}
              <b>{loc.name}</b>, {loc.city} —{longDate(r.dateKey)},{" "}
              {(r.slots || [r.slot]).join(" & ")}
            ...
            >
              ...
              >
                {t("venue.confirmation.calendar", {
                  defaultValue: "In den Kalender übernehmen",
                })}
              </button>
            </div>
          </div>
          ...
          >
            ...
            >
              {t("venue.confirmation.moreTables", {
                defaultValue: "Weitere Tische ansehen",
              })}
            </button>
          </div>
        </div>
    ...
    <div className="venue-detail-page">
      <div className="venue-detail-container">
        ...
        >
          ...
          <span className="tag">
            {seatCount}{" "}
            {t("venue.seats.oneTable", {
              defaultValue: "Plätze · ein Tisch",
            })}
          </span>
        </div>
              ...
              <div className="mini-reihe">
                {fotos.map((f, i) => (
                  <img
                    key={f.id}
                    src={f.klein || f.gross}
                    alt={`${t("venue.gallery.image", {
                      defaultValue: "Bild",
                    })} ${i + 1} ${t("venue.gallery.show", {
                      defaultValue: "anzeigen",
                    })}`}
                    className={`mini ${
                      i === fotoIdx ? "on" : ""
                    }`}
        ...
        {(activeAktionen(loc, date).length > 0 ||
          ...
          (() => {
            const aktionen = activeAktionen(
              loc,
              date,
            );

            const next = nextAktion(
              loc,
              dateKey(new Date()),
            );
                ...
                {aktionen.length > 0
                  ...
                  : next && (
                      <div className="aktion-box">
                        <span className="aktion-tag">
                          {t(
                            "venue.promotion.upcoming",
                            {
                              defaultValue:
                                "Kommende Aktionswoche",
                            },
                          )}
                        </span>
        ...
        {sonderEntry?.typ === "zu" ? (
          ...
          >
            ...
            {t("venue.closed.tableDays", {
              defaultValue: "Mischtisch-Tage hier:",
            })}{" "}
                ...
                {selected.length === 0
                  ...
                  : selected.length === 1
                    ? t("venue.contact.onePerson", {
                        defaultValue:
                          "Reservierung für 1 Person",
                      })
                    : t(
                        "venue.contact.multiplePeople",
                        {
                          count: selected.length,
                          defaultValue: `Reservierung für ${selected.length} Personen`,
                        },
                      )}
              </div>
              ...
              <div className="form-grid">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={t(
                      "venue.form.firstNamePlaceholder",
                      {
                        defaultValue: "Anna",
                      },
                    )}
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={t(
                      "venue.form.lastNamePlaceholder",
                      {
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={t(
                      "venue.form.emailPlaceholder",
                      {
                        defaultValue:
                          "anna@beispiel.example",
                      },
                    )}
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={t(
                      "venue.form.postcodePlaceholder",
                      {
                        defaultValue:
                          "01067 Dresden",
                      },
              ...
              >
                ...
                <span>
                  {t("venue.form.consent", {
                    venue: loc.name,
                    defaultValue: `Ich bin einverstanden, dass meine Angaben zur Bearbeitung dieser Reservierung gespeichert und an ${loc.name} übermittelt werden. *`,
                  })}{" "}
                  <button
                    type="button"
                    onClick={onRecht}
